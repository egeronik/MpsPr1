...
def prepare_info(res_dataset):
    ...
    max_info = f'ПО №{max_ind + 1} лучшее. Его счёт = {res_dataset[max_ind]:.4f}'
    min_info = f'ПО №{min_ind + 1} худшее. Его счёт = {res_dataset[min_ind]:.4f}'
    res = [f'По №{i + 1} счёт: {x:.4f}' for i, x in enumerate(res_dataset)]
    res.append(max_info)
    res.append(min_info)
    res = '\n'.join(res)
    print('---------Метод Топсис---------\n')
